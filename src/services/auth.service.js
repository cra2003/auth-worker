...
export async function getCurrentUser(c) {
	...
	try { email = await decryptData(user.email_cipher, encKey) } catch {}
	try { phone = user.phone_cipher ? await decryptData(user.phone_cipher, encKey) : null } catch {}
	try { addresses = user.addresses_cipher ? JSON.parse(await decryptData(user.addresses_cipher, encKey)) : [] } catch {}
	return c.json({
		user: {
...
export async function addAddress(c) {
	try {
		...
		if (user.addresses_cipher) {
			try { list = JSON.parse(await decryptData(user.addresses_cipher, encKey)) } catch {}
		}
